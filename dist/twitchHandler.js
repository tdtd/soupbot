...
TwitchHandler.prototype.getUsers = function(channel){
	...
	var url = 'https://tmi.twitch.tv/group/user/'+channel+'/chatters?client_id=liqeqlvo1tcoinseykent9zwug7246m';
...
TwitchHandler.prototype.getStreamInfo = function(channel){
	...
	var url = 'https://api.twitch.tv/kraken/streams/'+channel+'?client_id=liqeqlvo1tcoinseykent9zwug7246m';
	return new Promise(function(resolve, reject){
		request(url, function (error, response, body) {
			if (!error && response.statusCode == 200) {
				var bod = JSON.parse(body)
